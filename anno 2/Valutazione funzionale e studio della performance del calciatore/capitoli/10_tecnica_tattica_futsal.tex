...
Conoscere i principi di tecnica e di tattica dello sport in cui si lavora è importante per il
preparatore fisico, cioè per chi si occupa della performance atletica, per due ragioni:
\begin{itemize}
  \item avere un'\textbf{interazione corretta con l'allenatore}, che è il responsabile tecnico e con
    cui il preparatore deve dialogare per lavorare al meglio in campo;
  \item \textbf{trasferire alla partita} il lavoro svolto in campo.
\end{itemize}
Questi principi non servono al preparatore per sostituirsi all'allenatore o per strutturare da sé
situazioni tattiche, ma per \textbf{indirizzare il proprio lavoro} in una direzione che comprenda
anche gli aspetti cognitivi e la tattica --- individuale e di squadra --- della disciplina.

% ---------------------------------------------------------------------------
\section{L'impostazione dell'allenamento}

\rubrica{L'allenamento cognitivo}
Il calcio a 5 sottopone i giocatori a stimoli \textbf{costanti} e che si ripetono con \textbf{grande
frequenza}, da tutti i punti di vista: di conseguenza l'allenamento deve basarsi sulla
\textbf{componente cognitiva}.
  ...
  \item \textbf{con la palla}:
    \begin{itemize}
      ...
      \item esercitazioni ad alta intensità con scopo prettamente fisico --- per esempio
        ...
        di calcio a 5;
    \end{itemize}
  \item \textbf{senza palla}: i lavori per le capacità metaboliche --- aerobiche e anaerobiche ---
    e quelli prettamente neuromuscolari.
\end{itemize}
...
\begin{itemize}
  \item il \textbf{3-1}: tre giocatori sfalsati --- uno nella zona più centrale del campo e due in
    ampiezza sulle fasce --- più un \textbf{pivot} in avanti, che nel gergo dà la
    \textbf{profondità};
...
due letture:
\begin{enumerate}
  \item \textbf{ai lati e in avanti}: a destra o a sinistra per il gioco in \textbf{ampiezza}, in
    avanti per quello in \textbf{profondità};
  \item \textbf{dietro e avanti}: un passaggio \textbf{dietro di sé}, verso un compagno che gli
    garantisce sostegno o sicurezza, più \textbf{due appoggi} davanti a sé.
\end{enumerate}
...
\begin{itemize}
  \item nella \textbf{disposizione a tre} si ricerca la profondità attraverso il pivot, anche con il
    passaggio diretto;
  \item quando la \textbf{difesa avversaria è schierata molto bassa}, la fase di possesso si
    prolunga ed è più difficile trovare lo spazio per attaccare la porta;
  \item nella \textbf{disposizione a quattro} non c'è profondità iniziale: i quattro tendono ad
    \textbf{allinearsi} per poi attaccare la profondità che hanno liberato, con movimenti di
    smarcamento in profondità e in ampiezza.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{come si costruisce}: sono \textbf{schemi e movimenti predeterminati}, codificati e
    studiati durante l'allenamento;
  \item \textbf{perché serve}: ad alto livello --- internazionale, ma anche nazionale di alta
    qualificazione --- la pressione viene molto spesso portata \textbf{molto alta}, e le squadre
    sono costrette a studiare un modo per risolverla;
  \item \textbf{che cosa la caratterizza}: l'elevata \textbf{frequenza} di passaggi, controlli,
    smarcamenti, attacchi della profondità e \textbf{contromovimenti}. Sono qualità di tattica
    individuale e tecniche che fanno la differenza fra il giocatore di alto livello e gli altri.
\end{itemize}
...
\begin{itemize}
  \item \textbf{duello 1 contro 1}: su un campo di 40 $\times$ 20\,m si gioca \textbf{4 contro 4} di
...
Nascono quasi sempre da un \textbf{passaggio di possesso} fra le due squadre --- palla recuperata
con un contrasto, un intercetto o un semplice passaggio sbagliato --- da cui il termine
\textbf{transizioni}. Le più frequenti:
\begin{itemize}
  \item con tre giocatori coinvolti in fase di possesso: \textbf{3 contro 0} (la più frequente),
    poi \textbf{3 contro 1} e \textbf{3 contro 2};
  \item \textbf{1 contro il portiere}, in gergo il ``tu per tu'';
  \item \textbf{2 contro il portiere}, indicata come \textbf{2 contro 0}, e \textbf{2 contro 1}.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{gioco a 2 dai e segui}, dove l'elemento decisivo è lo \textbf{smarcamento}, e in
    particolare il \textbf{contromovimento} --- la finta con cui il giocatore trova lo spazio e il
    vantaggio che gli permettono di calciare;
  \item \textbf{gioco a 3 con pivot centrale} (dai e segui, con sovrapposizione in ampiezza) e
    \textbf{con pivot esterno}, che va a prendere la palla sull'esterno e gioca in parallela;
  \item \textbf{parallela e diagonale in forma ripetuta}: per smarcarsi, cioè per allontanarsi il
    più possibile dall'avversario e ricevere palla, i giocatori effettuano \textbf{cambi di
    direzione e di senso continui}, e sono costretti a \textbf{cambi di appoggio} e di baricentro
    costanti. È il collegamento diretto con la match analysis dal punto di vista cinematico.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{stop di pianta}: con la suola, cioè con la parte inferiore del piede; garantisce di
    fermare la palla e quindi di eseguire il gesto tecnico successivo nel modo più preciso possibile;
  \item \textbf{stop orientato di pianta}: è una scelta di \textbf{tattica individuale} --- il
    controllo di pianta non serve a fermare la palla ma a \textbf{spostarla}, per trovare nuove
    linee di passaggio o per smarcarsi dall'avversario;
  ...
  \item \textbf{trasmissione di pianta}: oltre che con tutte le parti del piede come nel calcio, la
    palla viene spesso trasmessa di pianta, e a volte \textbf{all'indietro di suola} per fintare e
    ingannare l'avversario;
  \item \textbf{trasmissione di esterno collo accompagnato}: serve ad \textbf{alzare il pallone},
    quindi a fare passaggi lunghi e alti che tagliano le linee difensive;
...
\begin{itemize}
  \item \textbf{conduzione di pianta}: con il controllo di suola il giocatore \textbf{prende il
    tempo} all'avversario diretto e trova lo spazio per calciare;
  \item giocare su una \textbf{superficie stabile e piatta} con un pallone a \textbf{rimbalzo
    controllato} facilita l'esecuzione di questi gesti;
  \item \textbf{controllo orientato}: serve a smarcarsi e a superare l'avversario per tirare in
    porta;
  \item \textbf{trasmissione di esterno collo accompagnato}: eseguita sia nel breve sia per servire
    una palla alta e lunga;
...
Analizzando il fermo immagine di un dribbling eseguito da un giocatore di altissimo livello, ciò che
interessa al preparatore fisico non è il gesto ma i \textbf{gradi articolari in cui va espressa
forza}:
\begin{itemize}
  \item l'atteggiamento di finta porta a una condizione coordinativa di \textbf{equilibrio
    estremamente instabile};
  \item \textbf{appoggio forte} sulla caviglia sinistra, con il \textbf{ginocchio opposto molto
    basso};
...
\begin{itemize}
  ...
  \item ogni operatore ha però le proprie convinzioni, e il modo \textbf{più efficace} per
    riprodurre la situazione specifica del calcio a 5 resta lavorare su \textbf{superfici rigide},
    cioè su quelle su cui il giocatore è abituato a giocare.
\end{itemize}
...
\begin{enumerate}
  \item il giocatore \textbf{controlla la palla di suola} con un piede --- per esempio il sinistro
    --- mentre è in appoggio sull'altro;
  \item \textbf{lascia andare la palla} dal piede che ha controllato, facendo sì che quello diventi
    il \textbf{nuovo appoggio};
  \item contemporaneamente esegue una \textbf{finta con il resto del corpo}, per eludere la
    marcatura del diretto avversario.
\end{enumerate}
Richiede capacità di equilibrio, \textbf{stiffness} muscolare e \textbf{forza esplosiva} nella
ripartenza successiva.
...
\begin{itemize}
  \item \textbf{rigore parato}: contro un tiro molto potente di collo interno, il portiere si sposta
    \textbf{rimanendo in posizione eretta}, per occupare il maggior volume possibile;
  \item \textbf{rigore realizzato}: eseguito di collo, con estrema potenza e con precisione.
\end{itemize}

\rubrica{Il tiro libero}
\begin{itemize}
  \item \textbf{esecuzione}: senza opposizione degli avversari, generalmente con un \textbf{tiro
    diretto} in porta;
  \item \textbf{tecnica del portiere}: posizione molto \textbf{avanzata} rispetto alla linea di
    porta; resta eretto fino all'ultimo per occupare il volume più ampio possibile e
    \textbf{indurre in errore} chi calcia, e solo allora effettua la parata;
  \item \textbf{vincolo regolamentare}: il portiere \textbf{non può avanzare} prima del tiro;
  \item la posizione avanzata non basta sempre: in un secondo esempio il portiere non riesce a
    opporsi a un tiro effettuato di interno piede.
\end{itemize}

\rubrica{Il calcio d'angolo}
\begin{itemize}
  \item primo schema: un giocatore effettua un \textbf{piccolo taglio davanti} per far passare il
    passaggio, e il tiro da fuori è facilitato anche dalla scelta tattica della difesa avversaria;
  \item secondo schema: schema predeterminato che porta al gol con un \textbf{passaggio diretto in
    area}.
\end{itemize}
...
porta allo \textbf{smarcamento} di un giocatore al limite dell'area, che va a segnare in maniera
diretta.

% ---------------------------------------------------------------------------
\section{Che cosa se ne ricava per il lavoro del preparatore}

Conoscere questi principi serve a costruire \textbf{insieme all'allenatore} esercitazioni di campo
che:
\begin{enumerate}
  \item abbiano come \textbf{obiettivo principale} quello fisico-atletico --- la sollecitazione
    della componente metabolica o di quella neuromuscolare;
  \item sollecitino \textbf{anche l'aspetto cognitivo}, dentro le conoscenze tecnico-tattiche del
