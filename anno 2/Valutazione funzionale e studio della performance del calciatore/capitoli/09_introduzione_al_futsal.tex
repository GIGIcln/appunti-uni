...
Senza conoscere le \textbf{regole} e l'\textbf{evoluzione del gioco} è difficile capire il modello
di prestazione del calcio a 5, e ancora meno come vada svolta la valutazione funzionale: è il
presupposto per profilare giocatore e squadra e programmare il processo di allenamento.
...
\end{enumerate}
Gli ultimi due titoli dicono ``valutazione \textbf{ed allenamento}'' perché il test non si esaurisce
in sé: serve a stabilire i \textbf{parametri dell'allenamento} per la qualità che ha indagato. Si
passa cioè dalla valutazione funzionale al lavoro sul campo.

% ---------------------------------------------------------------------------
\section{Che cos'è il calcio a 5}

\rubrica{Terminologia}
Nelle slide i termini \textbf{calcio a 5} e \textbf{futsal} si alternano:
\begin{itemize}
...
\begin{itemize}
  \item è uno \textbf{sport indoor}, che si gioca nei palazzetti su una \textbf{superficie rigida};
  \item presenta \textbf{aspetti in comune con il calcio}: tecnica e tattica individuale, principi
    di tattica collettiva e relativi sviluppi di gioco --- vale per esempio anche la semplice
    triangolazione fra due giocatori in situazione di superiorità numerica;
  \item ha però \textbf{caratteristiche proprie} che rendono specifica la disciplina: è il
...
\begin{enumerate}
  \item \textbf{fine anni Settanta -- inizio anni Ottanta}: nasce a \textbf{Roma}, nei circoli
    romani, dove si giocava \textbf{sui campi da tennis} togliendo la rete;
...
su scattano l'obbligo di giocare \textbf{indoor} e i vincoli sulle \textbf{dimensioni del campo}.

\rubrica{Il movimento femminile}
\begin{itemize}
  \item in Italia il calcio a 5 femminile \textbf{esiste da molti anni}, ma ha fatto passi da
    gigante nell'\textbf{ultimo quinquennio};
  \item il campionato di \textbf{Serie A} è nato nella stagione \textbf{2011/2012}; il
    \textbf{girone unico} è però recentissimo, in vigore da due stagioni;
  \item è oggi molto seguito, con profili di ottimo livello sia per \textbf{qualità} sia per
    \textbf{numero} di tesserate, e con un bacino in crescita fra le più piccole;
  \item in \textbf{Spagna} lo stesso percorso è avvenuto \textbf{prima} che in Italia: il movimento
    femminile vi è quindi assai più sviluppato.
\end{itemize}
...
\begin{itemize}
  \item dimensioni ideali: \textbf{40\,m di lunghezza $\times$ 20\,m di larghezza}, obbligatorie
  ...
  \item \textbf{conseguenza}: visti gli spazi ristretti, giocare su un campo più lungo o più largo
    ...
    fisico-atletico --- ma su quest'ultimo punto \textbf{non esistono studi};
  \item in campo ci sono complessivamente \textbf{10 giocatori}, cinque per squadra.
\end{itemize}

\rubrica{Il pallone}
Pallone \textbf{numero 4}, a \textbf{rimbalzo controllato}: lasciato cadere a terra rimbalza, ma
meno di un pallone da calcio.

\rubrica{Le superfici di gioco}
\begin{itemize}
  \item all'aperto: sintetico in erba;
  \item indoor: cemento, linoleum, plastica, gomma;
  \item \textbf{superficie ideale}: il \textbf{parquet}.
\end{itemize}

\subsection{Lo svolgimento della gara}

\rubrica{Le rimesse laterali}
Si eseguono \textbf{con i piedi}, ed è stato così esclusivamente per gli ultimi vent'anni. È una
regola \textbf{in evoluzione}: il \textit{board} FIFA si è riunito per rivedere alcune regole del
calcio a 5 e renderlo ancora più spettacolare, e fra le proposte c'è quella di lasciare al giocatore
la scelta fra piedi e mani.

\rubrica{I tiri liberi}
\begin{itemize}
  \item \textbf{che cosa sono}: la possibilità di calciare in porta \textbf{senza opposizione}
    dell'avversario, con palla ferma su un dischetto posto a \textbf{10\,m};
  \item \textbf{quando si ottengono}: ogni squadra dispone di \textbf{5 falli per tempo}; dal sesto
    in poi l'avversario non batte più la punizione nel punto del fallo, ma un tiro libero;
...
\begin{itemize}
  \item due tempi da \textbf{20 minuti effettivi}: il cronometro si ferma ogni volta che la palla
    non è in gioco;
  \item nelle categorie nazionali sono previsti \textbf{due arbitri} in campo, uno per metà campo,
    più un \textbf{cronometrista} a bordo campo incaricato di bloccare il tempo;
...
--- il giocatore che deve battere ha \textbf{4 secondi}, spesso contati ad alta voce dagli arbitri.
Scaduti i 4 secondi il pallone passa alla squadra avversaria.

\rubrica{I limiti del retropassaggio al portiere}
\begin{itemize}
  \item il portiere può toccare la palla \textbf{una sola volta} nella fase di possesso;
  \item dopo che l'ha giocata, i compagni \textbf{non possono più ridargli il pallone} finché questo
    non viene toccato da un avversario o non entra in possesso della squadra avversaria;
...
\begin{itemize}
  \item il giocatore espulso \textbf{esce definitivamente} dal terreno di gioco, esattamente come
    nel calcio: ``temporanea'' non si riferisce a lui;
  \item è l'\textbf{inferiorità numerica} a essere temporanea, e dura al massimo \textbf{2 minuti}:
    si gioca 5 contro 4;
  \item se la squadra in superiorità numerica segna, la parità si ristabilisce \textbf{subito},
    anche se i due minuti non sono trascorsi.
\end{itemize}
...
\begin{enumerate}
  ...
  \item il motivo principale è la possibilità di \textbf{semplificare l'insegnamento dei gesti
  ...
  \item da qui l'evidenza pratica: è più semplice controllare un pallone che \textbf{rimbalza poco}
    su una superficie \textbf{piatta e regolare} che un pallone che rimbalza molto su una superficie
    instabile come un campo in erba.
\end{enumerate}

\rubrica{Il modello brasiliano}
In Brasile i settori giovanili svolgono \textbf{allenamenti veri e propri di calcio a 5} --- non
allenamenti di calcio su un campo ristretto --- con istruttori e allenatori di calcio a 5. Non è un
caso che i giocatori brasiliani abbiano concetti di \textbf{tattica individuale} particolarmente ben
formati, primo fra tutti lo \textbf{smarcamento}: è un patrimonio appreso nel calcio a 5.
...
\begin{itemize}
  \item fino all'under 10 --- e a maggior ragione nelle categorie inferiori --- si fanno
    \textbf{solo} allenamenti di calcio a 5, \textbf{mai} di calcio;
  ...
  \item la progressione arriva a \textbf{sei sedute settimanali} all'under 14, essendo un club
    professionistico;
...
\begin{itemize}
  ...
  \item \textbf{non esiste il centrocampo}: le squadre o attaccano la porta avversaria o difendono
    la propria. Sono esattamente le \textbf{situazioni cruciali del calcio}, ma qui si ripetono di
    continuo: in ambito giovanile questo produce un numero elevatissimo di esperienze di quel tipo,
    ed è uno stimolo forte all'apprendimento;
  ...
  \item \textbf{il portiere di movimento}: un giocatore che ha tutti i privilegi del portiere ---
    fra cui giocare con le mani nella propria area --- ma che viene schierato per poter attaccare
    \textbf{5 contro 4} nella metà campo avversaria. Il vantaggio in fase di possesso è indubbio, ma
    in caso di palla recuperata dagli avversari la porta resta \textbf{sguarnita}, e la prima cosa
    che cercheranno è proprio la porta.
\end{itemize}
