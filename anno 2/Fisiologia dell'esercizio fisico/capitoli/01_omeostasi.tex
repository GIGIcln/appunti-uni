...
\begin{itemize}
  \item \textbf{feedback negativo}: tende a interrompere il senso di variazione della variabile,
    riportando il parametro verso il valore desiderato. È di gran lunga il caso prevalente: circa
    il \textbf{99\%} dei meccanismi fisiologici è a feedback negativo;
  \item \textbf{feedback positivo}: (citato dal docente tra i tipi di controllo a feedback, ma non
    definito né esemplificato).
\end{itemize}
...
L'\textbf{ipotalamo} è parte del sistema nervoso, ma è il punto in cui i due sistemi si saldano: è
il sistema nervoso a intervenire nei meccanismi anticipatori, mentre l'attività del sistema
endocrino è susseguente a quella nervosa.

\rubrica{Asse ipotalamo-ipofisi-surrene}
\begin{enumerate}
  \item l'ipotalamo produce il \textbf{CRF} (\textit{fattore rilasciante le corticotropine});
  \item il CRF agisce sull'\textbf{ipofisi}, prima ghiandola del sistema endocrino, che tra i molti
    ormoni produce l'\textbf{ACTH} (ormone adrenocorticotropo), molto importante per l'esercizio
    fisico;
  \item l'ACTH agisce sulla \textbf{corticale del surrene}, inducendola a produrre
    \textbf{cortisolo}, indicato come l'\textbf{ormone dello stress}, anche dello stress fisico:
    serve ad affrontare al meglio le prestazioni;
  \item in parallelo l'ipotalamo, attraverso il \textbf{sistema autonomo}, regola la
    \textbf{midollare del surrene}, che libera \textbf{catecolammine} (adrenalina e noradrenalina),
    i neurotrasmettitori che aiutano ad affrontare le prove impegnative, da un esame a una gara.
\end{enumerate}

\rubrica{Endorfine}
Sempre tramite il CRF, l'ipotalamo fa produrre le \textbf{endorfine}, molecole molto simili ai
peptidi oppioidi, che si occupano degli \textbf{adattamenti comportamentali} (paura, aggressività,
curiosità, vigilanza) e controllano anche il \textbf{dolore}.

I meccanismi che mettono in moto sistema nervoso ed endocrino sono i \textbf{medesimi} sia a riposo
sia sotto esercizio fisico o stress mentale.
